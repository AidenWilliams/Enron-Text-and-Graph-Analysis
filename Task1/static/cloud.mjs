...
export default function define(runtime, observer) {
    ...
    const fileAttachments = new Map([["dream.txt", new URL("/allUserClouds", import.meta.url)]]);
...
A demonstration of [d3-cloud](https://github.com/jasondavies/d3-cloud/). Paste into or edit the text below to update the chart. Note: word clouds [may be harmful](https://www.niemanlab.org/2011/10/word-clouds-considered-harmful/).`
    ...
    main.variable(observer("viewof source")).define("viewof source", ["html", "FileAttachment"], async function (html, FileAttachment) {
        ...
        const value = (await FileAttachment("dream.txt").json())
        console.log(value)
...
## Appendix`
    ...
    main.variable(observer("fontFamily")).define("fontFamily", function () {
        return (
            "sans-serif"
        )
    });
    main.variable(observer("fontScale")).define("fontScale", function () {
        return (
            15
        )
    });
    main.variable(observer("rotate")).define("rotate", function () {
        return (
            () => 0
        )
    });
    main.variable(observer("padding")).define("padding", function () {
        return (
            0
        )
    });
    ...
    main.variable(observer()).define(["words"], function (words) {
        return (
            words.filter(w => /\W/.test(w))
        )
    });
    main.variable(observer("data")).define("data", ["d3", "words"], function (d3, words) {
        return (
    ...
    main.variable(observer("stopwords")).define("stopwords", function () {
        return (
